	...
	public ArrayList<String> getArray() {
		...
		if (statementEncase.endsWith(":")) {
			...
			for (String cmd : this.arrayGet) {
				...
				// checks tab spaces
				String whiteSpaceCalc = StringUtils.getWhiteSpace(cmd);

				if (cmd.replaceAll("^\\s+", "").contains("CCU_COND_")) {
					newString = cmd.replace("CCU_COND_", "");
					newString = whiteSpaceCalc.substring(1) + "CCU_COND_" + newString.replace(whiteSpaceCalc, "");
				} else {
					// Puts CCU_COND_ before the thing
					newString = whiteSpaceCalc.substring(1) + "CCU_COND_" + cmd.replace(whiteSpaceCalc, "");
				}
